...
try:
    ...
    from .db_utils import get_db_connection, create_tables_if_not_exists, ALL_TABLE_SCHEMAS, \
                          get_last_imported_file_hash, upsert_imported_file_log 
except ImportError:
    # 如果相对导入失败 (通常是直接运行此脚本时)，则尝试修改 sys.path
    print("[IMPORT_SCRIPT_WARNING] 无法通过相对路径导入模块。尝试修改sys.path...")
    current_script_path = Path(__file__).resolve().parent # 当前脚本的父目录 (RandomBrainHole)
...
def parse_brainhole_excel(file_path: Path, source_file_name: str) -> Iterator[Dict[str, Any]]:
    ...
    for sheet_name in data_sheets: # 遍历每个数据工作表
        ...
        try:
            df = pd.read_excel(xls, sheet_name=sheet_name, header=None) # 读取时不指定表头
            if df.empty:
            ...
            for _, row in data_df.iterrows():
                ...
                if author == '——': author = '盐铁桶子' # 特殊处理
                win_rate_val = row.get('胜率', '暂无')
                win_rate_str = '暂无'
                if win_rate_val not in ['暂无', None, '']: 
                    try: win_rate_str = f"{float(win_rate_val) * 100:.1f}%" # 胜率格式化为百分比
...
def parse_fuzhipai_docx(file_path: Path, source_file_name: str) -> Iterator[Dict[str, Any]]:
    ...
    try:
            ...
            for run in para.runs: # 遍历段落中的文本片段 (run)
                if run.italic: # 如果是斜体
                    ...
                    if is_currently_italic:
                        ...
                        is_currently_italic = False
                    current_para_formatted_text += run.text
            if is_currently_italic: current_para_formatted_text += "]" # 处理段落末尾的斜体
        ...
        for card_text in cards_text_list:
            ...
            full_text_hash = calculate_text_sha256(card_text) # 计算卡牌全文的哈希值，用于唯一性检查
            yield {
                "card_title": title, 
                "full_text": card_text,
        ...
        if "File is not a zip file" in str(e) or "Package not found" in str(e):
             log_error(f"处理蝠汁牌文件 {source_file_name} 时出错: 文件可能不是有效的 .docx 格式 (例如是旧版 .doc)。请转换为 .docx 后重试。错误: {e}")
        else:
...
def parse_pinshi_excel(file_path: Path, source_file_name: str) -> Iterator[Dict[str, Any]]:
    ...
    try:
        ...
        actual_sheet_name = xls.sheet_names[0] if xls.sheet_names else "Sheet1" # 获取第一个工作表名
        df = pd.read_excel(xls, sheet_name=0, header=0) # 读取第一个工作表，第一行为表头
...
def insert_data_to_db(conn: sqlite3.Connection, table_name: str, data_iterator: Iterator[Dict[str, Any]]):
    ...
    batch_data = []; batch_size = 100 # 设置批量插入的大小
    for record_dict in data_iterator: # 遍历解析器产出的每条记录
        ...
        if len(batch_data) >= batch_size: #达到批处理大小时执行插入
            try:
                ...
                if cursor.rowcount < len(batch_data): # 如果受影响行数小于批大小，说明有记录被忽略
...
def main():
    ...
    try:
        ...
        if conn: conn.close(); globals()['_connection'] = None # 手动关闭并重置全局连接
        return
    ...
    for plugin_setting in plugin_cfg.plugins:
        ...
        for file_ext in plugin_setting.file_extensions:
            ...
            for file_path in data_folder.glob(f"*{file_ext}"):
                ...
                current_file_hash = calculate_file_sha256(file_path) # 计算当前文件的哈希值
                if current_file_hash is None: 
                    log_warning(f"    无法计算文件 {file_path.name} 的哈希值，将尝试处理，但可能导致重复导入。");
                
                last_hash = get_last_imported_file_hash(conn, file_identifier) # 从数据库获取上次导入的哈希值
                ...
                if data_iterator:
                    ...
                    if data_to_insert: # 如果确实有数据
                        log_info(f"    确认通过或无需确认，开始将 '{file_path.name}' 的数据插入表 '{target_table}'...")
                        # 将列表重新转为迭代器进行插入
                        ...
                        if current_file_hash: # 即使没有数据，也记录为已处理（如果哈希成功）
